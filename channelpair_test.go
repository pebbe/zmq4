...
import (
	zmqchan "github.com/abligh/zmqchan"
...
func runEcho(t *testing.T, num int, c *zmqchan.ChannelPair) {
	for {
		select {
...
func runWrite(t *testing.T, num int, c *zmqchan.ChannelPair) {
	tx := 0
	rx := 0
...
func TestZmqChanSocket(t *testing.T) {
	...
	if cb, err = zmqchan.NewChannelPair(sb, 0, 0); err != nil {
		t.Fatal("sb.NewChannelPair:", err)
	}
	sb = nil // don't access this or close it on defer

	if cc, err = zmqchan.NewChannelPair(sc, 0, 0); err != nil {
		t.Fatal("sb.NewChannelPair:", err)
	}
